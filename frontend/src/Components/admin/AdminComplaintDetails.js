...
export default function AdminComplaintDetails() {
    ...
    const fetchReporter = useCallback(async () => {
        try {
            const res = await api.get(`/api/complaint/${id}/reporter`);
    ...
    useEffect(() => {
        ...
        fetchCaseFiles();
        fetchReporter();
    }, [fetchComplaint, fetchInvestigators, fetchCaseFiles, fetchReporter]);
    ...
    return (
        <div style={styles.page}>
                ...
                <div style={styles.card}>
                    ...
                    {(!complaint.assignedTo || changingOfficer) && (
                        <>
                            ...
                            <button style={styles.primaryBtn} onClick={assignOfficer} disabled={assigning || closing}>
                                {complaint.assignedTo
                                    ? (assigning ? "Reassigning..." : "Reassign Investigator")
            ...
            <div style={{ ...styles.card, marginTop: 30 }}>
                <h3 style={styles.cardTitle}>Complainant Information</h3>
                <InfoRow label="Name" value={reporter?.fullName || "Not available"} />
                <InfoRow label="Email" value={reporter?.email || complaint.email} />
                <InfoRow label="Mobile" value={reporter?.mobileNo || "Not available"} />
                <InfoRow label="City" value={reporter?.city || "Not available"} />
                <InfoRow label="Address" value={reporter?.address || "Not available"} />
                <InfoRow label="Pincode" value={reporter?.pincode || "Not available"} />
            </div>
...
const ConfirmModal = ({ title, text, onCancel, onConfirm, loading = false }) => (
    <div style={styles.modalOverlay}>
        <div style={styles.modal}>
            ...
            <div style={styles.modalActions}>
                <button style={styles.secondaryBtn} onClick={onCancel} disabled={loading}>
                    Cancel
                </button>
                <button style={styles.primaryBtn} onClick={onConfirm} disabled={loading}>
